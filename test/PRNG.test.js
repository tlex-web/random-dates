...
class PRNG {
    ...
    constructor(seed) {
        ...
        this._mt = new Array(this._length)
        this._index = 0
        ...
        for (let i = 1; i < this._length; i++) {
            ...
            const prev = this._mt[i - 1]
            // Ensure that the value is different than the previous one
            this._mt[i] = (1812433253 * (prev ^ (prev >> 30)) + i) >>> 0
        }
    }
    ...
    generateNumbers() {
        ...
        for (let i = 0; i < this._length; i++) {
            const y = (this._mt[i] & 0x80000000) + (this._mt[(i + 1) % this._length] & 0x7fffffff) // 0x80000000 = 2147483648, 0x7fffffff = 2147483647
            this._mt[i] = this._mt[(i + 397) % 624] ^ (y >> 1)

            if (y % 2 !== 0) {
                this._mt[i] ^= 0x9908b0df // 0x9908b0df = 2567483615
            }
        }
    }
}
